...
def ray_triangle_intersection(orig, dir, vert0, vert1, vert2):
    # Implement the Moller-Trumbore intersection algorithm here
    edge1 = vert1 - vert0
    edge2 = vert2 - vert0
...
class Viewpoint:
    ...
    def get_visible_triangles(self, mesh):
        # Capture the depth image
        depth_image = self.capture_depth_image(mesh)

        # Convert the depth image to a point cloud
        depth_point_cloud = geometry.PointCloud.create_from_depth_image(
            depth_image,
            self.intrinsic,
            np.linalg.inv(np.eye(4))  # Assuming the depth image is already aligned with the viewpoint
        )

        # KDTree for fast nearest-neighbor search
        pcd_tree = geometry.KDTreeFlann(depth_point_cloud)

        # List to store indices of visible triangles
        visible_triangles = []

        # Iterate over all triangles in the mesh
        for i, triangle in enumerate(np.asarray(mesh.triangles)):
            # Use the centroid of the triangle as the search point
            centroid = np.mean(np.asarray(mesh.vertices)[triangle], axis=0)

            # Search for the nearest point in the point cloud
            [_, idx, _] = pcd_tree.search_knn_vector_3d(centroid, 1)

            if idx:
                # If a nearest point is found, the triangle is visible
...
# Capture depth image and print matrices with FOV-based projection
# viewpoint.capture_depth_image(mesh, projection_type='fov')


depth_image = viewpoint.capture_depth_image(mesh)
...
# viewpoint.visualize_point_cloud_from_depth()


# # Convert the depth image to a point cloud for debugging
...
# visible_triangles = viewpoint.get_visible_triangles(mesh)  # Uncomment this line when you add this method to your class
# print(visible_triangles)
